            ...
            <div class="row">
                <div class="col-12">
                    <div class="bread-inner">
                        <ul class="bread-list">
                            <p class="text-left p-4 text-white"><a href="{{route('home')}}">Trang chủ  ->  danh sách tin tức<i class="ti-arrow-right"></i></a></p>
                        </ul>
                    </div>
                </div>
            </div>
                                        ...
                                        <p class="date"><i class="fa fa-calendar" aria-hidden="true"></i> {{$post->created_at->format('d M, Y. D')}}
                                            <span class="float-right">
                                                <i class="fa fa-user" aria-hidden="true"></i>
                                                 {{$post->company->name_compn ?? 'Anonymous'}}
                                            </span>
                                        </p>
                                        <a href="{{route('news.detail',$post->slug)}}" class="title">{{$post->title}}</a>
                                        <p>{!!Str::limit($post->content, 200)!!}</p>
                                        <a href="{{route('news.detail',$post->slug)}}" class="more-btn">Đọc tiếp</a>
                                    </div>
                                </div>
                                <!-- End Single Blog  -->
                            </div>
